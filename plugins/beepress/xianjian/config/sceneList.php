  ...
  <script>
    if (navigator.userAgent.indexOf('Chrome') < 0) {
      ...
      window.location.href = phpUrl +
        '?page=beepress_xianjian_rec_options4'
    }
  </script>
  <meta charset="UTF-8">
  <title>先荐--场景列表</title>
...
<body class="paradigm_body">
  <div id='paradigm_scene_list' class='paradigm-app' v-cloak>
    <div class='_paradigm-content-plugIn'>
      ...
      <div class="_paradigm-content-plugIn-router">
        <div class="_paradigm-plug-scene-list">
          ...
          <div v-for="(item,index) in sceneList" :key="index" class="_paradigm-plug-scence-list-item">
            <div class="title-bar">
              ...
              <span class="wait-online-memo" v-if="item.status === statusSteps.step3_4 || item.status === statusSteps.step4">生成推荐结果中，通常需要1小时左右</span>
              <div class="operate-btn">
                <el-button @click.stop="viewCode(item.id)" v-if="showViewCode(item)">查看推荐栏代码</el-button>
                <el-button @click.stop="viewReport(item)" v-if="item.status >statusSteps.step3_4">查看报表</el-button>
        ...
        <el-dialog title="推荐栏自定义位置代码" :visible.sync="showSdk" id="plug-code-dialog-scope" @close='closedCodeDialog'>
          <div class="plug-code-dialog-content">
            <div class="item-box">
              ...
              <el-input :rows="10" type="textarea" ref="jsTextarea" v-model="pluginDiyCode" readonly=""></el-input>
        ...
        <!-- 运营规则设置 start -->
        <el-dialog title="配置规则" :visible.sync="showRuleDialog" class="operation-dialog" :close-on-click-modal='false'
          ...
          <div id="operation-dialog" v-if="showRuleDialog">
            ...
            <div>
              ...
              <el-card class="rule-type sort-rule sort-rule-info">
                ...
                <div class="rule-item">
                  <div class="rule-show-item rule-show-noborder-item" v-for="(item,index) in sortRuleList" :key="index">
                    <span class="sort-index-column">{{index+1}}</span>
                    <span class="sort-field-column">
                      <el-select class="rule-sort-field-select" v-model="item.field" placeholder="请选择特征" @change="changeSortField(item)">
                        <el-option v-for="item in  sortSelectList" :key="item.value" :label="item.name" :value="item.value"></el-option>
                      </el-select>
                    </span>
                    <span class="sort-field-value-column" v-if="item.field === 'category_v1'">
                      <el-select class="rule-sort-field-value-select" v-model="item.value" filterable placeholder="请选择特征值">
                        <el-option v-for="(item,index) in  categoryV1List" :key="index" :label="item" :value="item"></el-option>
                      </el-select>
                    </span>
                    <span class="sort-field-value-column" v-else>-</span>
                    <span class="sort-treatment-column">
                      <el-input-number v-model="item.treatment" :min=-10 :max=10 class="rule-input-num" @blur="checkSortInputNum(item,index)"
                    ...
                    <el-popover placement="right" width="200" trigger="hover" :content="sortAlertInfoList[item.field]"
                      popper-class="ruleEditTablePop" v-if="item.field">
                      <span class="sort-alert-column" slot="reference"><img class="sort-alert-grey" src="<?php  echo plugins_url('static/img/alert_grey.svg', __FILE__) ?>"
                          slot="reference" /></span>
                    </el-popover>
                  </div>
                  <div class="rule-show-item rule-show-noborder-item">
                    <div class="add-extract-feature link-primary add-diversity-rule" @click="addSortRule">
            ...
            <div v-if="currentSceneInfo && currentSceneInfo.scene && currentSceneInfo.scene.recommendType !== 1">
              ...
              <el-card class="rule-type diversity-rule diversity-rule-info">
                ...
                <div class="rule-item" v-for="(rule,groupIndex) in diversityRuleList" :key="groupIndex" v-if="rule.hidden!==1">
                  <div class="rule-show-item rule-show-noborder-item" v-for="(item,index) in rule.script" :key="item.id">
                    <span class="diversity-index-column">
                      {{index === 0 ? groupIndex-hiddenRuleExistIndex + 1 : '且'}}
                    </span>
                    <span class="diversity-field-column">
                      <el-select class="rule-diversity-field-select" v-model="item.field" placeholder="请选择特征" @change="changeField(item)">
                        <el-option v-for="item in  diversitySelectList" :key="item.value" :label="item.name" :value="item.value"></el-option>
                      </el-select>
                    </span>
                    <span class="diversity-condition-column">
                      <el-select class="rule-diversity-condition-select" v-model="item.op" placeholder="请选择条件" @change='changeOp(item)'>
                    ...
                    <span class="diversity-keyword-column" v-if="item.op === '=='">
                      <template v-if="item.field === 'publisher_id'">
                        <el-input v-model="item.value" class='diversity-keyword-input' placeholder="请输入关键字" :maxlength="20">
                    ...
                    <span class="diversity-limit-column" v-if="index===0">
                      <span>不超过</span>
                      <el-input-number v-model="rule.eValue" :min=1 :max="currentSceneInfo.scene.itemCount" class="rule-input-num diversity-limit-input-num"
                        @blur="checkDiversityInputNum(rule,groupIndex-hiddenRuleExistIndex)" ref="checkDiversityInputNum"></el-input-number>
                      <span>条</span>
                    </span>
                  </div>
                  <div class="rule-show-item rule-show-noborder-item ">
                    <div class="rule-opr-flex">
                      <div class="add-extract-feature link-primary add-content-rule" @click="addDiversityC(groupIndex)"
                        v-if="rule.script.length<3">
                        添加内容筛选条件
                      </div>
                    </div>

                    <el-button class="delete-button delete-group" @click="deleteDiversityRule(groupIndex)">
                      <i class="icon-delete"></i>
                      删除该组规则
                    </el-button>
                  </div>
                ...
                <div class="rule-show-item rule-show-noborder-item">
                  <div class="add-extract-feature link-primary add-diversity-rule" @click="addDiversityRule">
                    <i class="el-icon-circle-plus"></i>
                    添加一条多样性规则
                  </div>
                </div>
    ...
    new Vue({
        ...
        methods: {
          ...
          showAddScene(item) {
            let phpUrl = window.location.origin + window.location.pathname;
            window.location.href = phpUrl +
              '?page=beepress_xianjian_rec_options1&sceneNum=' + this.sceneList.length + '&sceneId=' + (
                item && item.id || '')
          },
          // 删除场景
          async deleteScene(item) {
            ...
            this.$confirm(lastSceneBeeFlg ? '推荐位有助于提升网站的留存率、访问次数，给访客提供更合适的内容，确认删除？' : '确认删除场景?',
                lastSceneBeeFlg ? '警告' :
                '提示', {
          ...
          viewReport(item) {
            let phpUrl = window.location.origin + window.location.pathname;
            window.location.href = phpUrl +
              '?page=beepress_xianjian_rec_options2&sceneId=' + (
                item && item.id || 0)
          ...
          goContentManage(item) {
            let phpUrl = window.location.origin + window.location.pathname;
            window.location.href = phpUrl +
              '?page=beepress_xianjian_rec_options3&sceneId=' + (
          ...
          closedCodeDialog() {
            ...
            if (showID) {
              let phpUrl = window.location.origin + window.location.pathname;
              window.location.href = phpUrl +
                '?page=beepress_xianjian_rec_options'
            }
          },
          // 获取自定义位置代码
